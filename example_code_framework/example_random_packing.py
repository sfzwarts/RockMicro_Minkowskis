...
name = f'Model_{seed}_pf_{packing_fraction:1.3f}'
resolution = 0.001
shape = 'circle'
extra = 1
beta = 'r'
...
## Another example of creating a random packing of circles with a constant diameter.
# Note, for this script you need PoreSpy
path = 'Data/random_packings/2D/homogeneoous_diameter/'
Scripts.microstructures.create_RP_porespy(f'{path}/circle_data', seed, packing_fraction, radius=0.005, edges='extended')

# Set the name of the text file containing the coordinates and mesh path
text_file = f'{path}/circle_data/pf_{packing_fraction:1.3f}/{name}.txt'
...
Scripts.handling_moose.run_simulation_mesh(mesh_file, output_name, moose_executable)