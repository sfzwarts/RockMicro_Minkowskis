...
def parse_template_mesh(mesh, output_name):
...
def call_MOOSE(data_file_path, moose_executable=None):
    
    if moose_executable == None:
        raise FileNotFoundError(
            "MOOSE executable not found. "
            "Provide 'moose_executable'"
        )

    command = f'{moose_executable} -i {data_file_path}'
...
def run_simulation_image(image, output_name, moose_executable=None):
    data_file_path = parse_template_image(image, output_name)
    call_MOOSE(data_file_path)

def run_simulation_mesh(mesh, output_name, moose_executable=None):
    data_file_path = parse_template_mesh(mesh, output_name)
    call_MOOSE(data_file_path, moose_executable)